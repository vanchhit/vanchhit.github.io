...
const TOPIC: TopicSlug = "ai-safety";

export const Route = createFileRoute("/blog/ai-safety")({
  head: () => {
    ...
    <div data-topic={TOPIC}>
      <header className={`border-b border-border/60 ${topicMotifClass(TOPIC)}`}>
        <div className="mx-auto max-w-6xl px-5 py-20">
          ...
          <div className="mt-6 font-mono text-xs uppercase tracking-widest" style={{ color: "var(--accent)" }}>Topic</div>
      ...
      <section className="mx-auto max-w-6xl px-5 py-16">
        {posts.length === 0 ? (
          ...
          <div className="divide-y divide-border/60">
            {posts.map((p) => (
              <Link key={p.slug} to="/blog/$slug" params={{ slug: p.slug }} data-topic={p.topic} className="group grid grid-cols-1 gap-2 py-6 sm:grid-cols-[120px_1fr] sm:gap-6">
                <div className="font-mono text-xs text-muted-foreground">{p.date}</div>
                <div>
                  <div className="font-display text-xl font-semibold tracking-tight transition-colors group-hover:text-accent">{p.title}</div>
